...
class App extends Component {
 ...
 todoadd(event) {
     event.preventDefault();
       ...
       makereq.open("POST", "https://cse204.work/todos", true);
  ...
  emptytodo(id){
    console.log(id);
       ...
       removereq.onreadystatechange = function(){
         if (this.readyState === 4 && this.status === 200) {
           const rtodo = s.state.todos.filter((todo) => {
             if (todo.id !== id){
               return todo;
             }
           });
       ...
       removereq.open("DELETE", "https://cse204.work/todos/" + id, true);
 ...
 render() {
   return (
     <section id= "itemstodo"> <NewTodo todoadd={this.todoadd} input={this.state.input} onChange={this.onChange}/>
     {this.state.todos.map((todo) =>
       <Todo key={todo.id} id={todo.id} completed={todo.completed}
       text={todo.text} emptytodo={this.emptytodo}/>
     )}
     </section>
 ...
 componentDidMount(){
   ...
   denotereq.open("GET", "https://cse204.work/todos", true);
